    ...
    try {
      ...
      if (!patient_id || !record_type || !title || !session_date) {
        return res.status(400).json({
          error:
            "Missing required fields: patient_id, record_type, title, session_date",
        });
      }
...
router.put(
  "/records/:id",
  requirePermission(["clinical_notes.update", "clinical_notes.edit_all"]),
  async (req, res) => {
    try {
      const { id } = req.params;
...
router.delete(
  "/records/:id",
  requirePermission("clinical_notes.delete"),
  async (req, res) => {
    try {
      const { id } = req.params;
