...
router.get('/patients/statistics', AuthMiddleware.verifyToken, AuthMiddleware.requirePermission('patients.read'), async (req, res) => {
  try {
...
router.get('/patients', AuthMiddleware.verifyToken, AuthMiddleware.requirePermission('patients.read'), async (req, res) => {
  try {
    const { 
      page = 1, 
      limit = 10, 
    ...
    if (search) {
      // Search by name
      patients = await Patient.searchByName(search, parseInt(limit), parseInt(offset));
      total = patients.length; // This is a simplified count for search
    } else {
      // Get all patients with filters
      patients = await Patient.findAll(parseInt(limit), parseInt(offset), filters);
...
router.get('/patients/:id', AuthMiddleware.verifyToken, AuthMiddleware.requirePermission('patients.read'), async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/patients/cf/:codice_fiscale', AuthMiddleware.verifyToken, AuthMiddleware.requirePermission('patients.read'), async (req, res) => {
  try {
...
router.get('/patients/ts/:numero_tessera_sanitaria', AuthMiddleware.verifyToken, AuthMiddleware.requirePermission('patients.read'), async (req, res) => {
  try {
    const { numero_tessera_sanitaria } = req.params;
    
    const patient = await Patient.findByNumeroTesseraSanitaria(numero_tessera_sanitaria);
    if (!patient) {
      return res.status(404).json({
        error: 'Patient not found'
      });
...
router.post('/patients', AuthMiddleware.verifyToken, AuthMiddleware.requirePermission('patients.write'), async (req, res) => {
  try {
    ...
    const existingPatientByCF = await Patient.findByCodiceFiscale(validation.cleaned.codice_fiscale);
    ...
    if (validation.cleaned.numero_tessera_sanitaria) {
      const existingPatientByTS = await Patient.findByNumeroTesseraSanitaria(validation.cleaned.numero_tessera_sanitaria);
    ...
    patientData.numero_tessera_sanitaria = validation.cleaned.numero_tessera_sanitaria;
...
router.put('/patients/:id', AuthMiddleware.verifyToken, AuthMiddleware.requirePermission('patients.write'), async (req, res) => {
  try {
    ...
    // Validate input
    const validation = PatientValidationUtils.validatePatientUpdate(updateData);
    if (!validation.isValid) {
    ...
    if (updateData.codice_fiscale && updateData.codice_fiscale !== patient.codice_fiscale) {
      const existingPatient = await Patient.findByCodiceFiscale(validation.cleaned.codice_fiscale);
      if (existingPatient) {
        return res.status(409).json({
          error: 'Patient with this Codice Fiscale already exists'
        });
    ...
    if (updateData.numero_tessera_sanitaria && updateData.numero_tessera_sanitaria !== patient.numero_tessera_sanitaria && validation.cleaned.numero_tessera_sanitaria) {
      const existingPatient = await Patient.findByNumeroTesseraSanitaria(validation.cleaned.numero_tessera_sanitaria);
      if (existingPatient) {
        return res.status(409).json({
          error: 'Patient with this Numero Tessera Sanitaria already exists'
        });
    ...
    if (updateData.numero_tessera_sanitaria !== undefined) {
      updateData.numero_tessera_sanitaria = validation.cleaned.numero_tessera_sanitaria;
    }
...
router.delete('/patients/:id', AuthMiddleware.verifyToken, AuthMiddleware.requirePermission('patients.delete'), async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/patients/:id/documents', AuthMiddleware.verifyToken, AuthMiddleware.requirePermission('patients.read'), async (req, res) => {
  try {
    const { id } = req.params;
