...
export const patientService = {
  ...
  getPatientByTS: async (numeroTesseraSanitaria) => {
    const response = await patientApi.get(`/patients/ts/${numeroTesseraSanitaria}`);
    return response.data;
  },
...
export const documentService = {
  uploadDocument: async (patientId, formData) => {
    const response = await patientApi.post(`/patients/${patientId}/documents`, formData, {
      headers: {
  ...
  getDocumentsByType: async (type, params = {}) => {
    const response = await patientApi.get(`/documents/types/${type}`, { params });
    return response.data;
  },
};
...
export const clinicalService = {
  ...
  getRecords: async (filters = {}) => {
    const response = await clinicalApi.get('/clinical/records', { params: filters });
    return response.data;
  },
  ...
  getPatientRecords: async (patientId, filters = {}) => {
    const response = await clinicalApi.get(`/clinical/records/patient/${patientId}`, { params: filters });
    return response.data;
  },
  ...
  updateRecord: async (id, recordData) => {
    const response = await clinicalApi.put(`/clinical/records/${id}`, recordData);
    return response.data;
  },
...
export const groupService = {
  ...
  getGroupMembers: async (groupId, activeOnly = true) => {
    ...
    const response = await groupApi.get(`/groups/${groupId}/members`, { params });
    return response.data;
  },

  addGroupMember: async (groupId, memberData) => {
    const response = await groupApi.post(`/groups/${groupId}/members`, memberData);
    return response.data;
  },

  updateGroupMember: async (groupId, memberId, memberData) => {
    const response = await groupApi.put(`/groups/${groupId}/members/${memberId}`, memberData);
    return response.data;
  },

  removeGroupMember: async (groupId, memberId) => {
    const response = await groupApi.delete(`/groups/${groupId}/members/${memberId}`);
    return response.data;
  },
...
export const billingService = {
  ...
  getInvoices: async (filters = {}) => {
    const response = await billingApi.get('/billing/invoices', { params: filters });
    return response.data;
  },
  ...
  updateInvoice: async (id, invoiceData) => {
    const response = await billingApi.put(`/billing/invoices/${id}`, invoiceData);
    return response.data;
  },

  updateInvoiceStatus: async (id, statusData) => {
    const response = await billingApi.put(`/billing/invoices/${id}/status`, statusData);
    return response.data;
  },
  ...
  getStatistics: async (filters = {}) => {
    const response = await billingApi.get('/billing/statistics', { params: filters });
    return response.data;
  },
  ...
  searchPatients: async (query) => {
    const response = await billingApi.get('/billing/patients/search', { params: { q: query } });
    return response.data;
  },
};
...
export const auditService = {
  ...
  getAuditStatistics: async (filters = {}) => {
    const response = await auditApi.get('/audit/statistics', { params: filters });
    return response.data;
  },

  getActiveUsers: async (limit = 10) => {
    const response = await auditApi.get('/audit/users/active', { params: { limit } });
    return response.data;
  },

  getEntityActivity: async (entityType, limit = 10) => {
    const response = await auditApi.get(`/audit/entities/${entityType}/activity`, { params: { limit } });
    return response.data;
  },
...
export const gdprService = {
  ...
  updateConsent: async (patientId, consentType, consentData) => {
    const response = await auditApi.put(`/gdpr/consents/${patientId}/${consentType}`, consentData);
    return response.data;
  },

  getConsentHistory: async (patientId, consentType = null) => {
    const params = consentType ? { consent_type: consentType } : {};
    const response = await auditApi.get(`/gdpr/consents/${patientId}/history`, { params });
    return response.data;
  },

  checkConsent: async (patientId, consentType) => {
    const response = await auditApi.get(`/gdpr/consents/${patientId}/${consentType}/check`);
    return response.data;
  },
  ...
  updateDataRequestStatus: async (requestId, status, notes = null) => {
    const response = await auditApi.put(`/gdpr/requests/${requestId}/status`, { status, notes });
    return response.data;
  },
