...
const ClinicalDiary = ({ patientId, clinicalRecordId, showAddButton = true }) => {
  ...
  const fetchEntries = async () => {
    try {
      ...
      // Fetch real clinical records data
      let recordsResponse;
      
      if (patientId) {
        // Get patient-specific clinical records
        recordsResponse = await clinicalService.getPatientRecords(patientId);
      } else if (clinicalRecordId) {
        // Get specific clinical record
        const recordResponse = await clinicalService.getRecord(clinicalRecordId);
      ...
      // Transform records to diary entries format
      ...
      console.error('Error fetching clinical diary entries:', error);
  ...
  const handleAddEntry = async () => {
    try {
      // Create new clinical record via API
      const recordData = {
  ...
  const handleEditEntry = async () => {
    try {
      ...
      const recordId = editEntry.id || editingId;
      if (!recordId) {
      ...
      const recordData = {
        ...
        description: editEntry.content,
        notes: editEntry.content,
        record_type: editEntry.type
      };
  ...
  return (
    <Box>
      ...
      <Dialog open={addDialogOpen} onClose={() => setAddDialogOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle>Nuova Nota Clinica</DialogTitle>
        <DialogContent>
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle>Modifica Nota Clinica</DialogTitle>
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 1 }}>
            <FormControl fullWidth>
              <InputLabel>Tipo di Nota</InputLabel>
